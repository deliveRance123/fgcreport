        ...
        if (empty($fullName) || empty($email)) {
            ...
            if ($chk->fetch()) {
                ...
                try {
                    ...
                    $upd = $db->prepare("UPDATE users SET full_name = ?, email = ?, phone = ?, bio = ?, profile_photo = ?, updated_at = NOW() WHERE id = ?");
                    $upd->execute([$fullName, $email, $phone, $bio, $photoPath, $uid]);
...
$roleLabels = [
    ...
    'zonal_admin'  => 'Zonal Secretary Admin',
    'church_admin' => 'Church Secretary Admin',
];
  ...
  <main class="main">
    ...
    <div class="profile-avatar-area">
      <?php if (!empty($user['profile_photo']) && file_exists(__DIR__ . '/' . $user['profile_photo'])): ?>
        <img src="<?= h($user['profile_photo']) ?>" alt="Profile Photo" style="width:80px;height:80px;border-radius:50%;object-fit:cover;border:3px solid var(--navy-soft);">
      <?php else: ?>
      ...
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-row">
          <div class="form-group full">
            ...
            <input type="file" id="profile_photo" name="profile_photo" accept="image/*" style="font-size:13px;">
            <small style="color:var(--ink-faint);font-size:11px;margin-top:4px;display:block;">Upload a profile picture so other users &amp; admins recognize you in Live Chat. Allowed: JPG, PNG, WEBP, GIF.</small>
          </div>
