    ...
    try {
        ...
        if ($role === 'church_admin') {
            ...
            if (!$previewUser) {
                ...
                $stmt = $db->prepare("INSERT INTO users (full_name, email, phone, password_hash, role, status) VALUES ('Preview Church Admin', 'preview_church@fgc-report.org', '123', ?, 'church_admin', 'active')");
            ...
            $_SESSION['email'] = 'preview_church@fgc-report.org';
            ...
            if (!$previewUser) {
                // Create user
                $passwordHash = password_hash('password', PASSWORD_DEFAULT);
                $stmt = $db->prepare("INSERT INTO users (full_name, email, phone, password_hash, role, status) VALUES ('Preview Zonal Admin', 'preview_zone@fgc-report.org', '123', ?, 'zonal_admin', 'active')");
                $stmt->execute([$passwordHash]);
                $userId = $db->lastInsertId();
            ...
            $_SESSION['full_name'] = $previewUser['full_name'];
            $_SESSION['email'] = 'preview_zone@fgc-report.org';
            ...
            if (!$previewUser) {
                // Create user
                $passwordHash = password_hash('password', PASSWORD_DEFAULT);
                $stmt = $db->prepare("INSERT INTO users (full_name, email, phone, password_hash, role, status) VALUES ('Preview Super Admin', 'preview_admin@report.org', '123', ?, 'super_admin', 'active')");
                $stmt->execute([$passwordHash]);
                $userId = $db->lastInsertId();
...
<title>Reports Portal Access</title>
...
<div class="auth-card">
  ...
  <div class="card-logo" style="justify-content: center; border-bottom: none; margin-bottom: 12px; padding-bottom: 0;">
    <div class="card-logo-text" style="text-align: center;">
      <svg width="42" height="42" viewBox="0 0 24 24" fill="none" stroke="#E31E24" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="background:#FFEBEB;padding:8px;border-radius:10px;margin:0 auto 12px;display:block;"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
      <span class="name" style="font-size: 16px; letter-spacing: -0.01em;">Reports Portal</span>
      <span class="sub">Secure System Entry</span>
    </div>
  </div>
