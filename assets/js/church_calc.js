/**
 * assets/js/church_calc.js — Live calculations for the Church Financial & Spiritual Report form.
 */
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Load due percentage rates from PHP global variable
    const rates = window.FGC_RATES || {};
    ...
    function isBaseFieldEmpty(fieldName) {
        const nEl = document.querySelector(`[name="${fieldName}_naira"]`);
        const kEl = document.querySelector(`[name="${fieldName}_kobo"]`);
    ...
    function updateDue(dueKey, fieldName) {
    ...
    function getRate(dueKey) {
        return rates[dueKey] !== undefined ? parseFloat(rates[dueKey]) : 0;
    }
    ...
    function calculateForm() {
        ...
        function getExpenseVal(itemKey) {
    ...
    allDuesInputs.forEach(input => {
        input.addEventListener('input', function() {
            ...
            const isNaira = input.name.endsWith('_naira');
            const prefix = input.name.substring(0, input.name.lastIndexOf('_'));
            const companionName = prefix + (isNaira ? '_kobo' : '_naira');
            const companion = document.querySelector(`[name="${companionName}"]`);
            if (companion) {
