...
interface Window {
  FGC_API_BASE?: string;
  FGC_IS_LOGGED_IN?: boolean;
  toggleFgcChatWidget?: () => void;
  switchFgcChatTab?: (tab: string) => void;
  submitAiChatQuery?: (e: Event) => void;
  sendQuickAiQuery?: (q: string) => void;
  openFgcLiveThread?: (partnerId: number, name: string, role: string, phone: string, photo: string, hasPhoto: boolean, isOnline: boolean) => void;
  backToFgcUserList?: () => void;
  submitLiveDirectMessage?: (e: Event) => void;
}

const fgcApiBase = window.FGC_API_BASE || 'chat-api';
let activeFgcPartnerId = 0;
let fgcPollingTimer: any = null;
let fgcHeartbeatTimer: any = null;
let fgcPartnerIsOnline = false;

// Helper to safely parse JSON response, ignoring ProFreeHost HTML bypass injections
function safeParseFgcJson(text: string): any {
...
function fgcFetch(url: string, options: RequestInit = {}): Promise<Response> {
...
function sendFgcHeartbeat(): void {
...
function toggleFgcChatWidget(): void {
    ...
    if (!win || !openIcon || !closeIcon) return;
...
function switchFgcChatTab(tab: string): void {
    ...
    if (!tabAi || !tabLive || !btnAi || !btnLive) return;
...
function submitAiChatQuery(e: Event): void {
...
function sendQuickAiQuery(q: string): void {
    const input = document.getElementById('fgcAiInput') as HTMLInputElement | null;
    if (input) {
        input.value = q;
...
function appendAiChatMessage(sender: 'user' | 'bot', text: string): void {
...
function loadFgcUserList(): void {
    ...
    .then(text => {
        ...
        if (data && data.success && Array.isArray(data.users)) {
            const online = data.users.filter((u: any) => u.is_online);
            const offline = data.users.filter((u: any) => !u.is_online);
            ...
            if (online.length > 0) {
                ...
                online.forEach((u: any) => { html += buildUserCard(u); });
            }
            ...
            if (offline.length > 0) {
                ...
                offline.forEach((u: any) => { html += buildUserCard(u); });
            }
...
function buildUserCard(u: any): string {
...
function openFgcLiveThread(partnerId: number, name: string, role: string, phone: string, photo: string, hasPhoto: boolean, isOnline: boolean): void {
    ...
    if (liveThreadEl) liveThreadEl.style.display = 'flex';
...
function backToFgcUserList(): void {
...
function submitLiveDirectMessage(e: Event): void {
    if (e && e.preventDefault) e.preventDefault();
    const input = document.getElementById('fgcLiveInput') as HTMLInputElement | null;
    if (!input || !activeFgcPartnerId) return;
...
function fetchFgcUnreadBadge(): void {
