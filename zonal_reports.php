...
// Enforce 1-Year Annual Subscription / Free Trial access control for zonal_admin
if ($role === 'zonal_admin' && !canUserCreateReport($_SESSION['user_id'])) {
    header("Location: zone-dashboard.php?error=" . urlencode("An active 1-Year Annual Subscription is required to access, create, or edit zonal reports. Please renew your subscription."));
    exit;
}
...
if ((!empty($_GET['reference']) || !empty($_GET['trxref'])) && !empty($zReport['id'])) {
    ...
    if (!empty($secKey)) {
        ...
        if ($ver['status']) {
            try {
                ...
                $ins = $db->prepare("INSERT INTO user_payments (user_id, report_id, report_type, payment_type, amount, reference, status, created_at) VALUES (?, ?, 'zonal', 'report_unlock', ?, ?, 'success', NOW()) ON DUPLICATE KEY UPDATE status = 'success'");
                $ins->execute([$userId, $rId, $paidAmt, $ref]);

                unlockReportDatabaseStatus($db, $rId, 'zonal');
                $db->commit();
...
if (!$zReport && !$viewOnly) {
    try {
        ...
        // Send email notification for zonal report creation
        if (isset($_SESSION['user_id'])) {
            $uStmt = $db->prepare("SELECT email, full_name FROM users WHERE id = ?");
            $uStmt->execute([$_SESSION['user_id']]);
            $userAcc = $uStmt->fetch();
            if ($userAcc && !empty($userAcc['email'])) {
                $zName = $zone['zone_name'] ?? 'Zone';
                $mName = monthName($month) . ' ' . $year;
                $msg = "Congratulations! You have initialized a new zonal monthly report for <strong>" . h($zName) . " Zone</strong> (" . h($mName) . ").";
                sendAppEmail($userAcc['email'], $userAcc['full_name'], "🎉 Zonal Report Initialized — " . $zName . " (" . $mName . ")", $msg, "zonal_reports.php?month={$month}&year={$year}", "Open Zonal Report");
            }
        }
    } catch (Exception $e) {
...
// Send Congratulations email if zonal report printing/exporting is triggered
if ((isset($_GET['notify_print']) || isset($_GET['pdf'])) && isset($_SESSION['user_id'])) {
    $sessionPrintKey = "printed_zonal_report_{$zoneId}_{$month}_{$year}";
    if (empty($_SESSION[$sessionPrintKey])) {
        ...
        if ($userAcc && !empty($userAcc['email'])) {
            $zName = $zone['zone_name'] ?? 'Zone';
            $mName = monthName($month) . ' ' . $year;
            $msg = "Congratulations! Your zonal monthly report for <strong>" . h($zName) . " Zone</strong> (" . h($mName) . ") has been successfully printed / exported from your portal.";
            sendAppEmail($userAcc['email'], $userAcc['full_name'], "🎉 Zonal Report Printed / Exported — " . $zName . " (" . $mName . ")", $msg, "zonal_reports.php?month={$month}&year={$year}", "View Zonal Portal");
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$viewOnly) {
    ...
    try {
        ...
        if ($action === 'submit') {
            ...
            // Send congratulations email on submission
            if (isset($_SESSION['user_id'])) {
                $uStmt = $db->prepare("SELECT email, full_name FROM users WHERE id = ?");
                $uStmt->execute([$_SESSION['user_id']]);
                $userAcc = $uStmt->fetch();
                if ($userAcc && !empty($userAcc['email'])) {
                    $zName = $zone['zone_name'] ?? 'Zone';
                    $mName = monthName($month) . ' ' . $year;
                    $msg = "Congratulations! Your zonal monthly report for <strong>" . h($zName) . " Zone</strong> (" . h($mName) . ") has been officially <strong>submitted</strong>. Thank you for your leadership!";
                    sendAppEmail($userAcc['email'], $userAcc['full_name'], "🎉 Zonal Report Submitted — " . $zName . " (" . $mName . ")", $msg, "zonal_reports.php?month={$month}&year={$year}", "View Zonal Report");
                }
            }
        }
...
function printReport() {
    fetch('zonal_reports.php?notify_print=1&month=<?= $month ?>&year=<?= $year ?>').catch(()=>{});
    window.print();
}
</script>
...
function payToUnlockZonalReport() {
    openFgcCheckoutModal('report_unlock', 'Unlock Submitted Zonal Report for Editing', <?= (float)($paySettings['report_unlock_fee'] ?? 2000) ?>, <?= $reportId ?>, 'zonal');
}
</script>
