    ...
    if ($action === 'add_custom_item') {
        ...
        if (!empty($customLabel)) {
            try {
                ...
                $stmt = $db->prepare("INSERT INTO church_expense_items (church_id, report_id, item_key, label, amount, is_custom, display_order) VALUES (?, ?, ?, ?, 0.00, 1, ?)");
                $stmt->execute([$churchId, $reportId, $itemKey, $customLabel, $order]);
                $successMsg = 'Custom expense item added!';
            } catch (Exception $e) {
                $errorMsg = 'Error adding item: ' . $e->getMessage();
            }
        }
    } elseif ($action === 'rename_item') {
        // Rename an expense item label
        $itemId = (int)$_POST['rename_id'];
        $newLabel = trim($_POST['rename_label'] ?? '');
        if ($itemId > 0 && !empty($newLabel)) {
            try {
                // Make sure it belongs to this church/report
                $stmt = $db->prepare("UPDATE church_expense_items SET label = ? WHERE id = ? AND church_id = ? AND (report_id = ? OR report_id IS NULL)");
                $stmt->execute([$newLabel, $itemId, $churchId, $reportId]);
                $successMsg = 'Expense item renamed successfully!';
            } catch (Exception $e) {
                $errorMsg = 'Error renaming: ' . $e->getMessage();
            }
        }
        ...
        try {
            ...
            $singleFields = [
                ...
                'prev_above_18', 'prev_under_18', 'prev_total',
                'new_above_18', 'new_under_18', 'new_total',
                'before_withdrawal_above_18', 'before_withdrawal_under_18', 'before_withdrawal_total',
                'withdrawn_total_above_18', 'withdrawn_total_under_18', 'withdrawn_total_total',
                'after_withdrawal_above_18', 'after_withdrawal_under_18', 'after_withdrawal_total'
            ];
  ...
  <div id="renameModal" class="modal" style="display:none; position:fixed; z-index:1000; left:0; top:0; width:100%; height:100%; background:rgba(0,0,0,0.5); align-items:center; justify-content:center;">
    <div style="background:#fff; padding:20px; border-radius:8px; width:340px; box-shadow:0 4px 15px rgba(0,0,0,0.2);">
      ...
      <form method="POST" action="">
          ...
          <input type="hidden" name="rename_id" id="renameItemId">
          <div class="mb-3">
              <label class="form-label small fw-semibold">New Label Description</label>
              <input type="text" name="rename_label" id="renameLabelInput" class="form-control form-control-sm" required>
          </div>
    ...
    <div style="background:#fff; padding:20px; border-radius:8px; width:340px; box-shadow:0 4px 15px rgba(0,0,0,0.2);">
      ...
      <form method="POST" action="">
          ...
          <div class="mb-3">
              ...
              <input type="text" name="custom_label" class="form-control form-control-sm" required placeholder="e.g. Youth Day Expenses">
          </div>
          <div class="d-flex justify-content-end gap-2">
